...
//Error	C4996	'gethostbyname': Use getaddrinfo() or GetAddrInfoW() instead or define _WINSOCK_DEPRECATED_NO_WARNINGS to disable deprecated API warnings	Ping	c : \users\john.gnew\source\repos\ping\ping\ping.cpp	136
...
//	bzero((char *)&whereto, sizeof(struct sockaddr));
	...
	memset((void *)&whereto, NULL, sizeof(struct sockaddr));
	to->sin_family = AF_INET;
//	to->sin_addr.s_addr = inet_addr(av[0]);
	inet_pton(AF_INET, av[0], (PVOID) to->sin_addr.s_addr);
	if (to->sin_addr.s_addr != (unsigned)-1)
	{
		strcpy_s(hnamebuf, av[0]);
		hostname = hnamebuf;
	}
	else {
...
//			bcopy(hp->h_addr, (caddr_t)&to->sin_addr, hp->h_length);
			memcpy((hp->h_addr), ((void *)&to->sin_addr), (hp->h_length));
			hostname = hp->h_name;
		}
		else {
		...
		printf("WSAStartup() looks fine!\n");
	{
		for (i = 0; i <= 10; i++)
		{
			...
			if (servptr == NULL)
				printf("\ngetprotobyname() - %s failed with error code %ld\n", protocol[i], WSAGetLastError());
			else
			{
	...
	if (options & SO_DEBUG) {
		if (pingflags & VERBOSE)
			printf("...debug on.n");
		setsockopt(s, SOL_SOCKET, SO_DEBUG, (const char *) &on, sizeof(on));
	}
	if (options & SO_DONTROUTE) {
		if (pingflags & VERBOSE)
			printf("...no routing.n");
		setsockopt(s, SOL_SOCKET, SO_DONTROUTE, (const char *) &on, sizeof(on));
	}
